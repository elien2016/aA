...
def start_at_craiglockhart
  ...
  execute(<<-SQL)
    SELECT
      DISTINCT stops_b.name, a.company, a.num
    FROM
      routes a
    JOIN
      routes b ON (a.company = b.company AND a.num = b.num)
...
def craiglockhart_to_sighthill
  ...
  execute(<<-SQL)
    SELECT
      DISTINCT a.num, a.company, stops_b.name, c.num, c.company
    FROM
      routes a
    JOIN
      routes b ON (a.company = b.company AND a.num = b.num)
    JOIN
      routes c ON ((b.company != c.company OR b.num != c.num) AND b.stop_id = c.stop_id)
    JOIN
    ...
    JOIN
      stops stops_b ON b.stop_id = stops_b.id
    JOIN
      stops stops_c ON c.stop_id = stops_c.id
    JOIN
      stops stops_d ON d.stop_id = stops_d.id
    WHERE
      stops_a.name = 'Craiglockhart' AND stops_d.name = 'Sighthill'
  SQL
end
